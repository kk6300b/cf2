...
inline PBYTE detour_gen_jmp_immediate(PBYTE pbCode, PBYTE pbJmpVal, bool DrvMode)
{
    PBYTE pbJmpSrc = pbCode + 5;
    if (DrvMode) {
        //SendPacket("BYTE", (ULONG64)(&*(pbCode++)), "E9");
        //SendPacket("INT32", (ULONG64)(&*((INT32*&)pbCode)++), (INT32)(pbJmpVal - pbJmpSrc));
        //DbgPrint("Ð´Èë int33");
    }
    else {
        *pbCode++ = 0xE9;   // jmp +imm32
...
inline PBYTE detour_gen_jmp_indirect(PBYTE pbCode, PBYTE *ppbJmpVal, bool DrvMode)
{
    PBYTE pbJmpSrc = pbCode + 6;
    if (DrvMode) {
        //SendPacket("BYTE", (ULONG64)(&*(pbCode++)), "FF");
        //SendPacket("BYTE", (ULONG64)(&*(pbCode++)), "25");
        //SendPacket("INT32", (ULONG64)(&*((INT32*&)pbCode)++), (INT32)((PBYTE)ppbJmpVal - pbJmpSrc));
        //DbgPrint("Ð´Èë ff25");
        //DrvWrite<BYTE>((ULONG64)(&*(pbCode++)), 0xff);
        //DrvWrite<BYTE>((ULONG64)(&*(pbCode++)), 0x25);
        //DrvWrite<INT32>((ULONG64)(&*((INT32*&)pbCode)++), (INT32)((PBYTE)ppbJmpVal - pbJmpSrc));
    }
    else {
        *pbCode++ = 0xff;   // jmp [+imm32]
        *pbCode++ = 0x25;
...
inline PBYTE detour_gen_brk(PBYTE pbCode, PBYTE pbLimit, bool DrvMode)
{
    while (pbCode < pbLimit) {
        if (DrvMode) {
            //DbgPrint("Ð´Èë 0xCC");
            //SendPacket("BYTE", (ULONG64)(&*(pbCode++)), "CC");
            //*pbCode++ = 0xcc;
            //DrvWrite<BYTE>((ULONG64)(&*(pbCode++)), 0xcc);
        }
        else {
            *pbCode++ = 0xcc;   // brk;
            //SendPacket("bbbbas");
...
static DWORD detour_writable_trampoline_regions()
{
    ...
    for (PDETOUR_REGION pRegion = s_pRegions; pRegion != NULL; pRegion = pRegion->pNext) {
        DWORD dwOld;
        if (!VirtualProtectEx((HANDLE)-1,pRegion, DETOUR_REGION_SIZE, PAGE_EXECUTE_READWRITE, &dwOld)) {
            return GetLastError();
        }
    }
    return NO_ERROR;
...
static void detour_runnable_trampoline_regions()
{
    ...
    for (PDETOUR_REGION pRegion = s_pRegions; pRegion != NULL; pRegion = pRegion->pNext) {
        DWORD dwOld;
        VirtualProtectEx((HANDLE)-1, pRegion, DETOUR_REGION_SIZE, PAGE_EXECUTE_READ, &dwOld);
...
LONG WINAPI DetourTransactionAbort()
{
    ...
    for (DetourOperation *o = s_pPendingOperations; o != NULL;) {
        ...
        DWORD dwOld;
        VirtualProtectEx((HANDLE)-1, o->pbTarget, o->pTrampoline->cbRestore,
                       o->dwPerm, &dwOld);
...
LONG WINAPI DetourTransactionCommit(bool DrvMode = false)
{
    return DetourTransactionCommitEx(NULL, DrvMode);
}
...
LONG WINAPI DetourTransactionCommitEx(_Out_opt_ PVOID **pppFailedPointer,bool DrvMode)
{
...
#ifdef DETOURS_X64
            detour_gen_jmp_indirect(o->pTrampoline->rbCodeIn, &o->pTrampoline->pbDetour, DrvMode);
            PBYTE pbCode = detour_gen_jmp_immediate(o->pbTarget, o->pTrampoline->rbCodeIn, DrvMode);
            pbCode = detour_gen_brk(pbCode, o->pTrampoline->pbRemain, DrvMode);
...
#undef DETOURS_EIP
    ...
    for (o = s_pPendingOperations; o != NULL;) {
        // We don't care if this fails, because the code is still accessible.
        DWORD dwOld;
        VirtualProtectEx((HANDLE)-1, o->pbTarget, o->pTrampoline->cbRestore, o->dwPerm, &dwOld);
...
LONG WINAPI DetourAttach(_Inout_ PVOID *ppPointer,
                         _In_ PVOID pDetour,bool DrvMode = false)
{
    return DetourAttachEx(ppPointer, pDetour, NULL, NULL, NULL, DrvMode);
}
//LONG WINAPI DetourAttach(_Inout_ PVOID* ppPointer,
//    _In_ PVOID pDetour, PDETOUR_TRAMPOLINE *back)
//{
//    return DetourAttachEx(ppPointer, pDetour, back, NULL, NULL);
//}
LONG WINAPI DetourAttachEx(_Inout_ PVOID *ppPointer,
                           _In_ PVOID pDetour,
                           _Out_opt_ PDETOUR_TRAMPOLINE *ppRealTrampoline,
                           _Out_opt_ PVOID *ppRealTarget,
                           _Out_opt_ PVOID *ppRealDetour,bool DrvMode)
{
    LONG error = NO_ERROR;
...
#ifdef DETOURS_X64
    pbTrampoline = detour_gen_jmp_indirect(pbTrampoline, &pTrampoline->pbRemain, DrvMode);
    pbTrampoline = detour_gen_brk(pbTrampoline, pbPool, DrvMode);
#endif // DETOURS_X64
    ...
    DWORD dwOld = 0;
    //VirtualProtectEx((HANDLE)-1, pbTarget, cbTarget, PAGE_EXECUTE_READWRITE, &dwOld);
    if (!VirtualProtectEx((HANDLE)-1, pbTarget, cbTarget, PAGE_EXECUTE_READWRITE, &dwOld)) {
        error = GetLastError();
        DETOUR_BREAK();
        goto fail;
    }
                         ...
                         _In_ PVOID pDetour)
{
    ...
    DWORD dwOld = 0;
    if (!VirtualProtectEx((HANDLE)-1, pbTarget, cbTarget,
