        ...
        if (DetourVirtualProtectSameExecute(pder->pinh, pder->cbinh,
            ...
            if (pder->pclr != NULL && !fUpdated32To64) {
                if (DetourVirtualProtectSameExecute(pder->pclr, pder->cbclr,
                    ...
                    VirtualProtectEx((HANDLE)-1, pder->pclr, pder->cbclr, dwPermClr, &dwIgnore);
                    fSucceeded = TRUE;
                }
            }
            else {
            ...
            VirtualProtectEx((HANDLE)-1, pder->pinh, pder->cbinh, dwPermInh, &dwIgnore);
        }
        VirtualProtectEx((HANDLE)-1,pder->pidh, pder->cbidh, dwPermIdh, &dwIgnore);
    }
